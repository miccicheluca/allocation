...
class HRP:
    def __init__(
        ...
        self.df_reference = self._pivot_data(
            df=self._compute_rets(
                df_ref=df_ref.sort(["date", "asset_name"]),
            )
        )
        ...
        df_ref_temp = (
            infos.df_reference.filter(
                pl.col("date").is_between(
                    infos.rebal_date
                    - dt.timedelta(weeks=int(4 * args_corr.n_month_delay)),
                    infos.rebal_date,
                )
            )
            .sort("date")
    ...
    def _compute_df_ref_temp(
        ...
        return self.df_reference.select(pl.col(["date"] + rebal_assets))
